 ...
 * Key Expansion
 */
static void key_expansion(uint8_t *key, uint8_t *w)
{
    uint8_t tmp[4];
    uint8_t i;
    uint8_t len = Nb * (Nr + 1);
...
static void cipher(uint8_t *in, uint8_t *out, uint8_t *w)
{
    uint8_t state[4 * Nb];
    uint8_t r, i, j;
...
static uint8_t w[Nb * (Nr + 1) * 4] = {0}; // expanded key
